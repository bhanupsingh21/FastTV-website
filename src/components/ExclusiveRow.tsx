...
const ExclusiveCard = ({ show, onClick }: { show: Show; onClick: () => void }) => {
    ...
    return (
        ...
        >
            ...
            <motion.div
                className="relative w-[230px] h-[407px] flex-shrink-0 rounded-[12px] overflow-hidden border border-white/10 bg-[#1a1a1a]"
    ...
    return (
        <div className="pt-4 pb-12 space-y-6 relative group/row z-20">
            <div className="container mx-auto px-6 flex justify-between items-center mb-6">
                <h2 className="text-[24px] font-bold text-white flex items-center gap-2 group cursor-pointer whitespace-nowrap">
                    {title}
                </h2>
            ...
            <div className="relative">
                <div
                    ref={rowRef}
                    className="flex overflow-x-auto gap-[20px] pb-8 pt-4 scrollbar-hide scroll-smooth no-scrollbar px-6 xl:px-[calc((100vw-1280px)/2+24px)] 2xl:px-[calc((100vw-1536px)/2+24px)]"
                >
                    {shows.map((show) => (
                        <ExclusiveCard
                            key={show.id}
